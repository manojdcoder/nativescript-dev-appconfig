...
module.exports = function ($logger, $projectData, $usbLiveSyncService) {
	return new Promise(function (resolve, reject) {
		...
		// overwrite config from argv
		_.each(appConfigData, function (value, key) {
			var argvKey = key.toLowerCase().replace('_', '-');
			if (_.has(config, argvKey)) {
				var newValue;
				if (_.isBoolean(appConfigData[key])) {
					newValue = config[argvKey].toLowerCase() === 'true';
				} else {
					newValue = config[argvKey];
				}
				appConfigData[key] = newValue;
			}
		});
